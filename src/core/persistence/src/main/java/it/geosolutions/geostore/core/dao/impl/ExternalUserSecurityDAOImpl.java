...
public class ExternalUserSecurityDAOImpl extends SecurityDAOImpl {
        ...
        for (SecurityRule rule : rules) {
            ...
            if (rule.getGroupname() != null) {
                ...
                if (rule.getGroup() == null) {
                    ...
                    filledRule.setGroup(rule.getGroup());
                }
            } else if (rule.getGroup() != null) {
                filledRule.setGroup(rule.getGroup());
            }
    ...
    private SecurityRule[] extractNames(SecurityRule[] rules) {
        ...
        for (SecurityRule rule : rules) {
            ...
            if (rule.getGroup() != null) {
                UserGroup group = rule.getGroup();
                if (group.getGroupName() != null)
                    extractedRule.setGroupname(rule.getGroup().getGroupName());
                if (group.getId() != null && group.getId().longValue() != -1l)
                    extractedRule.setGroup(group);
            } else {
    ...
    private boolean addGroupRule(SecurityRule rule, List<String> groupNames) {
        String groupName = null;
        if (rule.getGroupname() != null)
            groupName = rule.getGroupname();
        else if (rule.getGroup() != null)
            groupName = rule.getGroup().getGroupName();
        return groupName != null && groupNames.contains(groupName);
    }

    @Override
    public boolean remove(SecurityRule entity) {
        if (entity.getId() != null && entity.getId().longValue() != -1l)
            return super.removeById(entity.getId());
        else return super.remove(entity);
    }
}
